...
namespace LetsBeRationalLib {
    internal class NormalDistribution {
        ...
        private const double norm_cdf_asymptotic_expansion_first_threshold = -10.0;
        private static double norm_cdf_asymptotic_expansion_second_threshold = -1 / Math.Sqrt(Double.Epsilon);
        ...
        internal static double norm_cdf(double z) {
            if (z <= norm_cdf_asymptotic_expansion_first_threshold) {
                ...
                if (z >= norm_cdf_asymptotic_expansion_second_threshold) {
                    ...
                    do {
                        ...
                        ++i;
                        a = Math.Abs(a);
                    } while (lasta > a && a >= Math.Abs(sum * Double.Epsilon));
                }

                return -norm_pdf(z) * sum / z;
            }

            return 0.5 * ErfcCody.erfc_cody(-z * ONE_OVER_SQRT_TWO);
        }

        internal static double norm_pdf(double x) { return ONE_OVER_SQRT_TWO_PI * Math.Exp(-.5 * x * x); }

        internal static double inverse_norm_cdf(double u) {
            //
